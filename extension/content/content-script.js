(() => {
  ...
  async function getSettings() { const r = await chrome.storage.local.get({ [SETTINGS_KEY]: { extensionEnabled: true } }); return { extensionEnabled: true, ...(r[SETTINGS_KEY] || {}) }; }
  async function getRule(id) { const r = await chrome.storage.local.get({ [keyRule(id)]: undefined }); return r[keyRule(id)]; }
  async function getRules() {
    const c = context(); const r = await chrome.storage.local.get({ [keyDomain(c.domain)]: [], [keyPage(c.domain, c.path)]: [] });
    const ids = [...new Set([...(r[keyDomain(c.domain)] || []), ...(r[keyPage(c.domain, c.path)] || [])])]; if (!ids.length) return [];
    const loaded = await chrome.storage.local.get(ids.map(keyRule)); return ids.map(id => loaded[keyRule(id)]).filter(Boolean);
  }
  async function saveRule(rule) {
    const dKey = keyDomain(rule.domain), pKey = keyPage(rule.domain, rule.path || '/'); const old = await chrome.storage.local.get({ [dKey]: [], [pKey]: [] });
    await chrome.storage.local.set({ [keyRule(rule.ruleId)]: rule, [dKey]: [...new Set([...(old[dKey] || []), rule.ruleId])], [pKey]: [...new Set([...(old[pKey] || []), rule.ruleId])] }); return rule;
  }
  async function updateRule(rule) { return saveRule({ ...rule, updatedAt: new Date().toISOString() }); }
  async function deleteRule(id) {
    const rule = await getRule(id); if (!rule) return; await chrome.storage.local.remove(keyRule(id));
    const dKey = keyDomain(rule.domain), pKey = keyPage(rule.domain, rule.path || '/'); const old = await chrome.storage.local.get({ [dKey]: [], [pKey]: [] });
  ...
  function nthOfType(el) { const p = el.parentElement; if (!p) return 1; return [...p.children].filter(x => x.tagName === el.tagName).indexOf(el) + 1; }
  function selectorFor(el, id, classes) { if (id) return `#${CSS.escape(id)}`; const tag = el.tagName.toLowerCase(); if (classes.length) return `${tag}.${classes.slice(0, 2).map(CSS.escape).join('.')}`; return `${tag}:nth-of-type(${nthOfType(el)})`; }
  async function fingerprint(el) {
    const ids = stableTokens([el.id]), classes = stableTokens(String(el.className || '').split(/\s+/).filter(Boolean)), attrs = await semanticAttrs(el), text = normalizeText(el.textContent || '');
    ...
    const chain = []; let p = el.parentElement, depth = 0; while (p && depth++ < 4) { chain.push({ tag: p.tagName.toLowerCase(), stableClasses: stableTokens(String(p.className || '').split(/\s+/).filter(Boolean)) }); p = p.parentElement; }
    fp.ancestorContext = { chain, depthCaptured: chain.length }; const parent = el.parentElement, siblings = parent ? [...parent.children] : [], i = siblings.indexOf(el);
    ...
    const r = el.getBoundingClientRect(), vw = Math.max(innerWidth, 1), vh = Math.max(innerHeight, 1); fp.geometricHint = { viewportXRatio: Math.max(0, Math.min(1, r.x / vw)), viewportYRatio: Math.max(0, Math.min(1, r.y / vh)), widthRatio: Math.max(0, Math.min(1, r.width / vw)), heightRatio: Math.max(0, Math.min(1, r.height / vh)) }; return fp;
  }
  ...
  async function score(fp, el, cssMatched) {
    ...
    const i = el.parentElement ? [...el.parentElement.children].indexOf(el) : -1, structure = fp.structureContext?.siblingSignature?.indexWithinStableParent === i ? 1 : 0, css = cssMatched ? 1 : 0;
    ...
    const independent = INDEPENDENT.filter(k => parts[k].available && parts[k].score >= .65).length; return { element: el, totalScore: available ? total / available : 0, independent, parts };
  }

  const shadowRoots = new Set();
  function collectShadowRoots() {
    const roots = [document]; const visit = root => { for (const el of root.querySelectorAll('*')) if (el.shadowRoot && !shadowRoots.has(el.shadowRoot)) { shadowRoots.add(el.shadowRoot); roots.push(el.shadowRoot); visit(el.shadowRoot); } }; visit(document); for (const root of shadowRoots) if (!roots.includes(root)) roots.push(root); return roots;
  }
  ...
  async function match(fp) {
    const set = new Set(), cssSet = new WeakSet(); if (fp.cssSelector) queryAll(fp.cssSelector).forEach(el => { set.add(el); cssSet.add(el); }); if (fp.stableId?.value) queryAll(`#${CSS.escape(fp.stableId.value)}`).forEach(el => set.add(el));
    for (const a of fp.semanticAttributes || []) if (a.valueKind === 'structural') queryAll(`[${CSS.escape(a.name)}="${CSS.escape(a.value)}"]`).forEach(el => set.add(el)); if (!set.size) queryAll(fp.tagName || '*').slice(0, MAX_CANDIDATES).forEach(el => set.add(el));
    const ranked = []; for (const el of set) { ranked.push(await score(fp, el, cssSet.has(el))); if (ranked.length >= MAX_CANDIDATES) break; } ranked.sort((a, b) => b.totalScore - a.totalScore || b.independent - a.independent); const a = ranked[0], b = ranked[1];
    if (!a || a.totalScore < .6) return { status: 'notFound', confidence: a?.totalScore || 0, selected: a }; if (a.totalScore < .85 || a.independent < 3 || Math.abs(a.totalScore - (b?.totalScore || 0)) <= .05) return { status: 'ambiguous', confidence: a.totalScore, selected: a }; return { status: 'active', confidence: a.totalScore, selected: a };
  }
  ...
  function removeRule(id) { queryAll(`[${ATTR}]`).forEach(el => { if (el.getAttribute(ATTR) !== id) return; const o = original.get(el); if (o) { el.className = o.className; if (o.blur) el.style.setProperty('--pb-blur', o.blur); else el.style.removeProperty('--pb-blur'); if (o.strong) el.style.setProperty('--pb-strong-blur', o.strong); else el.style.removeProperty('--pb-strong-blur'); original.delete(el); } else ['pb-effect-base','pb-effect-blur','pb-effect-strongBlur','pb-effect-pixelate','pb-effect-blackout','pb-effect-hide'].forEach(c => el.classList.remove(c)); el.removeAttribute(ATTR); }); }
  ...
  let selection = false, hover = null, retryTimer = null, evaluating = false; const retryCounts = new Map(), pageSuppressed = new Set();
  ...
  async function evaluateRule(rule) { if (!rule.enabled || !(await getSettings()).extensionEnabled) { removeRule(rule.ruleId); return { status: 'disabled', confidence: 0 }; } if (pageSuppressed.has(rule.ruleId)) { removeRule(rule.ruleId); return { status: 'suppressed', confidence: 0 }; } const result = await match(rule.fingerprint); removeRule(rule.ruleId); if (result.status === 'active' && result.selected?.element) apply(result.selected.element, rule); await updateRule({ ...rule, status: result.status, lastConfidence: result.confidence, statusContext: { domain: context().domain, path: context().path, evaluatedAt: new Date().toISOString() }, lastMatchedAt: result.status === 'active' ? new Date().toISOString() : rule.lastMatchedAt }); return result; }
  async function evaluateAll() { if (evaluating) return; evaluating = true; try { const rules = await getRules(), pending = []; for (const r of rules) { const result = await evaluateRule(r); if (r.enabled && !pageSuppressed.has(r.ruleId) && (result.status === 'notFound' || result.status === 'ambiguous')) pending.push(r.ruleId); } if (pending.length) schedule(pending); } finally { evaluating = false; } }
  function schedule(ids) { clearTimeout(retryTimer); retryTimer = setTimeout(async () => { const next = []; for (const id of ids) { if (pageSuppressed.has(id)) continue; const n = retryCounts.get(id) || 0; if (n >= MAX_RETRIES) continue; retryCounts.set(id, n + 1); const r = await getRule(id); if (!r || !r.enabled) continue; const result = await evaluateRule(r); if (result.status !== 'active') next.push(id); } if (next.length) schedule(next); }, RETRY_MS); }

  const observedRoots = new WeakSet();
  function observeRoot(root) { if (!root || observedRoots.has(root)) return; const observer = new MutationObserver(() => { collectShadowRoots(); observeAllRoots(); if (!evaluating) { clearTimeout(retryTimer); retryTimer = setTimeout(evaluateAll, RETRY_MS); } }); observer.observe(root, { childList: true, subtree: true, attributes: true }); observedRoots.add(root); }
  function observeAllRoots() { observeRoot(document.documentElement); for (const root of shadowRoots) observeRoot(root); }

  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => { (async () => { switch (message?.type) {
    case 'BG_ENTER_SELECTION': startSelection(); break; case 'BG_EXIT_SELECTION': stopSelection(); break;
    ...
    default: break;
  } sendResponse({ ok: true }); })().catch(err => sendResponse({ ok: false, error: String(err?.message || err) })); return true; });

  function installSpaHooks() { for (const name of ['pushState', 'replaceState']) { const originalFn = history[name]; if (originalFn.__progettoBlurWrapped) continue; const wrapped = function (...args) { const before = location.href; const result = originalFn.apply(this, args); if (location.href !== before) { retryCounts.clear(); pageSuppressed.clear(); clearTimeout(retryTimer); retryTimer = setTimeout(evaluateAll, RETRY_MS); } return result; }; Object.defineProperty(wrapped, '__progettoBlurWrapped', { value: true }); history[name] = wrapped; } addEventListener('popstate', () => { retryCounts.clear(); pageSuppressed.clear(); clearTimeout(retryTimer); retryTimer = setTimeout(evaluateAll, RETRY_MS); }, true); }
